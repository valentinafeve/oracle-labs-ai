...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE EVENT_RAW PURGE';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
...
-- Tabla de staging para EVENT.
-- Se recrea en cada corrida y mantiene el mismo orden de columnas del CSV,
-- incluyendo MATCH_DATE al final.
...
BEGIN
  ...
  SELECT
    ...
    TO_DATE(TRIM(MATCH_DATE) DEFAULT NULL ON CONVERSION ERROR, 'DD-MON-RR'),
    TEAM_ID,
  ...
  -- El CSV original de PREDICT_BY_ANGLE termina con una ultima columna vacia
  -- en cada fila (por ejemplo: 1859,2,"",), lo que provoca rechazos en
  -- DBMS_CLOUD.COPY_DATA. Como los valores reales del archivo son secuenciales
  -- y las columnas PREDICTED_GOAL y XG vienen nulas, se genera la misma tabla
  -- directamente en SQL para asegurar una carga estable.
  INSERT INTO PREDICT_BY_ANGLE (
    ID, ANGLE, PREDICTED_GOAL, XG
  )
  SELECT
    1858 + LEVEL,
    LEVEL * 2,
...
Puedes descargar los cinco CSV directamente desde el repositorio:

- [event.csv](https://raw.githubusercontent.com/valentinafeve/oracle-labs-ai/main/07-workshop-AI-Database-Agent-Factory/data-premier/event.csv)
- [match.csv](https://raw.githubusercontent.com/valentinafeve/oracle-labs-ai/main/07-workshop-AI-Database-Agent-Factory/data-premier/match.csv)
- [player_stats.csv](https://raw.githubusercontent.com/valentinafeve/oracle-labs-ai/main/07-workshop-AI-Database-Agent-Factory/data-premier/player_stats.csv)
- [predict_by_angle.csv](https://raw.githubusercontent.com/valentinafeve/oracle-labs-ai/main/07-workshop-AI-Database-Agent-Factory/data-premier/predict_by_angle.csv)
- [xg_matrix.csv](https://raw.githubusercontent.com/valentinafeve/oracle-labs-ai/main/07-workshop-AI-Database-Agent-Factory/data-premier/xg_matrix.csv)
